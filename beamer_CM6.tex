...
$$\tilde{\mu} &= \sum\limits_{j=1}^J \tilde {w_j}\overline{y_{:j}} \text{ with } \tilde {w_j} \propto (\hat{\sigma}_A^2 + \frac{\hat{\sigma}_\epsilon^2}{n})^{-1}$$
\end{itemsize} 
\end{frame}
...
\begin{frame}
\begin{itemize}\setlength{\itemsep}{5pt}
\visible<1->{\item \textbf{Vraisemblance} :
\\
...
\vfill
\centering{C'est l'estimateur $\hat\mu$ qu'on avait propos\'e avant.}
